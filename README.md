...
Built by [irlab.uk](https://irlab.uk) (Nick Bennett) & [Phil Warren](https://philwarrenphotography.com)
...
- **Colour Calibration** — rotates each colour primary's hue and adjusts its saturation independently, closer to how a camera colour profile actually works than hue-band targeting is. Useful for correcting a sensor's own spectral response, or fine-tuning a channel swap without touching the matrix itself. Includes a shadow-only tint.
- **Triple Band Aerochrome** — a transform measured directly from a real triple-bandpass filter stack, not derived from film. Replaces the channel swap rather than layering on top of it. Alongside it, an Aerochrome 1443 tone-response curve digitised from Kodak's own published characteristic curves — gamma about 3.1, with the film's three dye layers staggered by up to a stop, which is what shifts the colour balance with exposure rather than just the brightness.
...
- **Sharpening** — two-scale unsharp mask (fine detail and coarse structure weighted separately) plus a 5×5 deconvolution pass and edge-direction-aware anti-aliasing, so diagonal edges don't stair-step at high amounts. Radius and an edge-detection mask (to keep sharpening off flat sky and out of noise) are both exposed rather than hidden behind the amount slider.
- **Clarity** — mid-tone local contrast, distinct from Contrast: it compares each pixel to a wide-radius blur of its neighbourhood rather than remapping tonal values globally, so it responds to actual spatial structure. Negative values soften rather than crush, deliberately capped so it never drives shadows to black.
- **Noise reduction** — an edge-aware bilateral filter with a second Tukey-bisquare pass that rejects outliers (hot pixels) without pulling down the whole local average, plus a separate chrominance-space pass that stays out of luma edges so denoising doesn't bleed colour across a boundary.
- **Dehaze** — recovers contrast and saturation lost to atmospheric haze using a regional dark-channel estimate, or, pulled the other way, adds a gentle atmospheric veil for a moodier look.
- **Halation** — the red-orange glow real film shows around bright highlights, from light scattering back through the base and re-exposing the red-sensitive layer. Most visible on genuine specular highlights — windows, reflections, sun through leaves.
- **Highlights and shadows** — both rebuilt around a small local reference (not just the pixel's own value): highlights recovery restores the texture a global pull-down would otherwise flatten; shadow lift and shadow crush use different curve shapes on purpose, since a lift needs to protect noisy blacks from being amplified and a crush does not.
- **Contrast pivots on the image's own mean brightness**, not a fixed midpoint — measured from a histogram pass on load, so a bright, foliage-heavy IR frame and a dark interior both get an S-curve that bends around where their tones actually sit rather than a value chosen for neither.
...
- **`.cube` LUT import and export** — export bakes your exact look into a standards-compliant 3D LUT, verified against the tool's own LUT reader, and prompts for a filename before saving rather than downloading silently under an auto-generated one
