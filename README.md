...
- **Raw sensor data** — reads past the embedded preview into the sensor plane itself: Bayer pattern, black level, as-shot white balance, and (for CR2, or any format with AMaZE decode switched on) the actual demosaiced sensor pixels rather than the camera's own JPEG rendering
...
- **RAW support** — CR2 decodes straight from the sensor by default (a hand-written Bayer decoder, auto white-balanced since a raw decode arrives nothing like a camera JPEG), with the embedded preview available as a faster fallback; NEF/CR3/ARW/DNG and the rest load through their embedded preview; 16-bit TIFF imports at full resolution with no compression artefacts
- **AMaZE sensor decode** *(opt-in)* — a second, higher-quality decoder for CR2 and other RAW formats, built on LibRaw (sensor unpack) and RawTherapee's AMaZE (edge-directed demosaic — interpolates along the actual detail in the image instead of averaging blindly, avoiding the zippering a simpler decoder shows on fine detail). Runs in a Web Worker via WebAssembly, only loaded when switched on. This component is GPL-3 rather than this project's own AGPL-3.0 — see [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)
