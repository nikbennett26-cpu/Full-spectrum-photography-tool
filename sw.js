...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    const cached = await caches.match(req);
    if (cached) {
      /* Stale while revalidate, properly this time: the earlier version of
         this comment claimed the trailing .catch() covered a failure here,
         but it didn't — caches.open(...).then(c => c.put(req, res.clone()))
         was never RETURNED into the outer chain, so it ran as its own
         detached promise with no error handler attached at all. Whatever
         causes res.clone() to throw (a real race is possible when the same
         URL is requested twice in close succession — exactly what happens
         when a module and its wasm both get fetched during one page load —
         each fetch(req) here gets its own independent Response, but if
         something upstream ends up reading one before this callback runs,
         clone() throws "body is already used"), that throw now has
         somewhere to go instead of surfacing as an uncaught rejection. */
      (async () => {
        try {
        ...
        } catch (e) { /* background revalidation failing is never fatal — the page already has `cached` */ }
      })();
      return cached;
    }
